...
describe("GET /api/deliverables/[id]/download", () => {
  ...
  it("downloads DB-backed text content as an attachment", async () => {
    allowHiveAndReturn();
    const res = await GET(request("/api/deliverables/wp-1/download"), { params: Promise.resolve({ id: "wp-1" }) });
  ...
  it("rejects file paths outside the hive workspace", async () => {
    ...
    allowHiveAndReturn(row({ content: null, file_path: outside, workspace_path: workspace, mime_type: "text/plain" }));
    try {
      const res = await GET(request("/api/deliverables/wp-1/download"), { params: Promise.resolve({ id: "wp-1" }) });
