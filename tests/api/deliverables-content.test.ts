...
describe("GET /api/deliverables/[id]/content", () => {
  ...
  it("serves DB-backed text content", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/deliverables/wp-1/content"), { params: Promise.resolve({ id: "wp-1" }) });
    expect(res.status).toBe(200);
    expect(await res.text()).toBe("# DB content");
  });

  it("serves HTML content with a sandbox CSP", async () => {
    mocks.sql.mockResolvedValueOnce([row({ content: "<script>parent.alert('nope')</script>", filename: "page.html", mime_type: "text/html", render_mode: "html" })]);
    const res = await GET(new Request("http://localhost/api/deliverables/wp-1/content"), { params: Promise.resolve({ id: "wp-1" }) });
    expect(res.status).toBe(200);
  ...
  it("serves files only when contained in the hive workspace", async () => {
    ...
    mocks.sql.mockResolvedValueOnce([row({ content: null, file_path: filePath, workspace_path: workspace, mime_type: "text/plain" })]);
    try {
      const res = await GET(new Request("http://localhost/api/deliverables/wp-1/content"), { params: Promise.resolve({ id: "wp-1" }) });
      expect(res.status).toBe(200);
