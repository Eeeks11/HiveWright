...
describe("<AgentCard>", () => {
  ...
  it("renders role + title and opens SSE to /api/tasks/:id/stream", () => {
    render(<AgentCard hiveId="hive-test" taskId="t-1" assignedTo="dev-agent" title="Build X" />);
    expect(screen.getByText("dev-agent")).toBeTruthy();
    expect(screen.getByText("Build X")).toBeTruthy();
    expect(MockEventSource.instances[0].url).toBe("/api/tasks/t-1/stream?hiveId=hive-test");
  });

  it("wraps the card in a link to /tasks/:id for clickthrough", () => {
    render(<AgentCard hiveId="hive-test" taskId="t-abc" assignedTo="dev-agent" title="Build X" />);
  ...
  it("shows 'Waiting for output…' until the first chunk arrives", () => {
    render(<AgentCard hiveId="hive-test" taskId="t-2" assignedTo="dev-agent" title="A" />);
    expect(screen.getByText(/Waiting for output/)).toBeTruthy();
  });

  it("appends stdout/stderr chunks and drops the placeholder", () => {
    render(<AgentCard hiveId="hive-test" taskId="t-3" assignedTo="dev-agent" title="A" />);
    act(() => {
  ...
  it("closes SSE on unmount", () => {
    const { unmount } = render(<AgentCard hiveId="hive-test" taskId="t-4" assignedTo="dev-agent" title="A" />);
