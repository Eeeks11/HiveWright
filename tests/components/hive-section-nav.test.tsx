...
describe("<HiveSectionNav>", () => {
  ...
  it("preserves hive-scoped destinations including Files and quality feedback", async () => {
    ...
    expect(screen.getByRole("link", { name: "Goals" }).getAttribute("href")).toBe("/goals?hiveId=hive-2");
    expect(screen.getByRole("link", { name: "Decisions" }).getAttribute("href")).toBe("/decisions?hiveId=hive-2");
