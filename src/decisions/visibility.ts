...
export const INTERNAL_DECISION_SQL = `(
  ...
  OR COALESCE(d.route_metadata #>> '{normalizedOwnerQuestion}', 'true') = 'false'
  ...
  OR LOWER(d.title) LIKE '%hive supervisor%'
  ...
  OR LOWER(d.context) LIKE '%outbound notifier%'
...
export const INTERNAL_DECISION_REASON_SQL = `
  CASE
    ...
    WHEN COALESCE(d.route_metadata #>> '{normalizedOwnerQuestion}', 'true') = 'false' THEN 'not_owner_normalized'
    ...
    WHEN LOWER(d.title) LIKE '%task quality%'
      ...
      OR LOWER(d.context) LIKE '%qa recheck%'
      OR LOWER(d.context) LIKE '%quality doctor%'
      OR LOWER(d.context) LIKE '%outbound notifier%' THEN 'legacy_internal_text'
