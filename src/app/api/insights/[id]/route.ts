...
export async function PATCH(
  ...
  try {
    ...
    if (!user.isSystemOwner) {
      const canMutate = await canMutateHive(sql, user.id, insight.hive_id as string);
      if (!canMutate) return jsonError("Forbidden: caller cannot manage this insight's hive", 403);
    }
