...
describe("POST /api/notifications/preferences access control", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRequireApiUser.mockResolvedValue({
      user: { id: "user-1", email: "user@example.com", isSystemOwner: false },
  ...
  it("returns 403 before inserting when the caller cannot manage the hive", async () => {
    ...
    const response = await POST(new Request("http://localhost/api/notifications/preferences", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ hiveId: "hive-1", channel: "email", config: {} }),
    }));
