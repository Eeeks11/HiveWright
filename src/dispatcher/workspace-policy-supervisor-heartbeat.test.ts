...
describe("supervisor heartbeat workspace policy classification", () => {
  it("does not classify supervisor heartbeat reports as code-changing just because they cite runtime/source evidence", () => {
    ...
    expect(evaluateTaskWorkspacePolicy(ctx({
      task,
      hiveSlug: "hivewright",
      projectWorkspace: "/home/operator/.hivewright/hives/hivewright/projects/runtime-health",
      baseProjectWorkspace: "/home/operator/.hivewright/hives/hivewright/projects/runtime-health",
      gitBackedProject: false,
    }))).toMatchObject({
      allowed: true,
  ...
  it("still blocks source-edit instructions that follow a supervisor findings section", () => {
    const decision = evaluateTaskWorkspacePolicy(ctx({
      task: {
        ...baseTask,
        assignedTo: "hive-supervisor",
        ...
        brief: [
          "## Hive Health Report",
          "### Findings",
          "- F-1: stalled implementation task quoted \"Patch the HiveWright dashboard source code and add Vitest tests\".",
  ...
  it("still blocks finding-id source-edit bullets under supervisor findings", () => {
    const decision = evaluateTaskWorkspacePolicy(ctx({
      task: {
        ...baseTask,
        assignedTo: "hive-supervisor",
        ...
        brief: [
          "## Hive Health Report",
          "### Findings",
          "- F-22: Patch the HiveWright dispatcher source code and add Vitest tests.",
        ].join("\n"),
        acceptanceCriteria: "Code changes and tests are committed.",
      },
      hiveSlug: "hivewright",
  ...
  it("still blocks finding-labeled source-edit bullets under supervisor findings", () => {
    const decision = evaluateTaskWorkspacePolicy(ctx({
      task: {
        ...baseTask,
        assignedTo: "hive-supervisor",
        ...
        brief: [
          "## Hive Health Report",
          "### Findings",
          "- Finding F-22: Patch the HiveWright dispatcher source code and add Vitest tests.",
        ].join("\n"),
        acceptanceCriteria: "Code changes and tests are committed.",
      },
      hiveSlug: "hivewright",
