...
const NEGATED_SOURCE_EDIT_PATTERN = /\b(do not|don't|without)\b.{0,30}\b(edit|change|modify|patch|write|touch|inspect or modify)\b.{0,100}\b(code|repo|repository|repositories|source|implementation code|source implementation|codebase|component|typescript|migration|schema|dashboard|api route)\b/i;
...
export function isCodeChangingTask(task: Pick<ClaimedTask, "assignedTo" | "title" | "brief" | "acceptanceCriteria">): boolean {
  ...
  const negatedSourceEditIntent = NEGATED_SOURCE_EDIT_PATTERN.test(text);
  const explicitSourceEditIntent = EXPLICIT_SOURCE_EDIT_PATTERN.test(text) && !negatedSourceEditIntent;
  const positiveSourceEditIntent = POSITIVE_SOURCE_EDIT_PATTERN.test(text) && !negatedSourceEditIntent;
