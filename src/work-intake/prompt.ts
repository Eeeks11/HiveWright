...
- Proof-only verification, audit, checklist, or report work with explicit no-code/no-file-change constraints still counts as a direct task
...
Input: "Verify auth coverage for the remaining goal mutation handlers. Audit those handlers and produce a concise implementation checklist with exact file paths. Do not modify application code."
Output: {"type":"task","role":"security-auditor","confidence":0.9,"reasoning":"Proof-only verification scope with explicit no-code instructions. One agent can audit the handlers and return the checklist in a single session."}
